...
args = argparse.ArgumentParser("ScanNet Evaluation Settings")
args.add_argument("--exp_name", default="scannet_mlp", type=str)
args.add_argument("--root_dir", type=str, default="../exps/")
args.add_argument("--out_dir", type=str, default="evaluation/scannet_mlp")
...
all_results_min_chamfer = []
all_results_max_f_score = []
output = []
for idx, scan in enumerate(scenes):
    ...
    for i in range(1, 20, 1):
        ...
        print(metrics)
        if metrics['Acc'] + metrics['Comp'] < min_chamfer:
            min_chamfer = metrics['Acc'] + metrics['Comp']
            evaluate_min_chamfer = metrics
            output_min_chamfer = ply_file
        if metrics['F-score'] > max_f_score:
            max_f_score = metrics['F-score']
            evaluate_max_f_score = metrics
            output_max_f_score = ply_file
    all_results_min_chamfer.append(evaluate_min_chamfer)
    all_results_max_f_score.append(evaluate_max_f_score)
    output.append([output_min_chamfer, output_max_f_score])
    
# print all results
for scan, metric in zip(scenes, all_results_min_chamfer):
    values = [scan] + [str(metric[k]) for k in metric.keys()]
    out = ",".join(values)
    print(out)

for scan, metric in zip(scenes, all_results_max_f_score):
    values = [scan] + [str(metric[k]) for k in metric.keys()]
    out = ",".join(values)
    print(out)

...
class ImplicitNetworkGrid(nn.Module):
    ...
    def gradient_occ(self, x):
        x.requires_grad_(True)
        y = self.forward(x)[:,1:2]
        y = torch.sigmoid(-10 * y)
        d_output = torch.ones_like(y, requires_grad=False, device=y.device)
        gradients = -torch.autograd.grad(
            outputs=y,
            inputs=x,
    ...
    def get_outputs(self, x):
        ...
        occ = torch.sigmoid(-10 * output[:,1:2])
        # change the feature vector range
        ...
        gradients_occ = -torch.autograd.grad(
            outputs=occ,
            inputs=x,
            grad_outputs=d_output,
            create_graph=True,
        ...
        return sdf, occ, feature_vectors, gradients_sdf, gradients_occ
...
class RenderingNetwork(nn.Module):
    def __init__(
        ...
        self.latent_active = latent_active
        self.decode_active = decode_active
        if color_encoder:
            dims = [d_in + feature_vector_size] + dims + [dims[-1]] + [d_out]
        else:
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, indices):
        ...
        if self.color_encoder:
            if self.latent_active == "sigmoid":
                # print("latent_active sigmoid")
                x = self.sigmoid(x)
            elif self.latent_active == "relu":
                # print("latent_active relu")
                x = self.relu(x)
            # else:
            #     print("Without latent_active")
    ...
    def decode_color(self, x):
        ...
        x = lin(x)
        if self.decode_active == "sigmoid":
            # print("decode_active sigmoid")
            color = self.sigmoid(x)
        elif self.decode_active == "tanh":
            # print("decode_active tanh")
            color = 0.5 * torch.tanh(x) + 0.5
        elif self.decode_active == "sine":
            color = 0.5 * torch.sin(x) + 0.5
            
        # color = torch.sin(x) * 0.5 + 0.5
        return color
...
class MonoSDFNetwork(nn.Module):
    ...
    def forward(self, input, indices):
        ...
        # with torch.no_grad():
        sdf, occ, feature_vectors, gradients_sdf, gradients_occ = self.implicit_network.get_outputs(points_flat)
        
        rgb_flat = self.rendering_network(points_flat, gradients_sdf, dirs_flat, feature_vectors, indices)
        rgb = rgb_flat.reshape(-1, N_samples, rgb_flat.shape[-1])

        weights_occ = self.volume_rendering_occ(z_vals, occ)
        rgb_values_occ = torch.sum(weights_occ.unsqueeze(-1) * rgb, dim=1)
        depth_values_occ = torch.sum(weights_occ * z_vals, 1, keepdim=True) / (weights_occ.sum(dim=1, keepdim=True) + 1e-8)
        depth_values_occ = depth_values_occ * depth_scale

        weights = self.volume_rendering(z_vals, sdf)

        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)
        
        if self.rendering_network.color_encoder:
            rgb_direct_decoder_flat = self.rendering_network.decode_color(rgb_flat)
            rgb_direct_decoder_flat = rgb_direct_decoder_flat.reshape(-1, N_samples, rgb_direct_decoder_flat.shape[-1])
            rgb_direct_decoder_values = torch.sum(weights.unsqueeze(-1) * rgb_direct_decoder_flat, 1)
            rgb_values = self.rendering_network.decode_color(rgb_values)
            rgb_values_occ = self.rendering_network.decode_color(rgb_values_occ)
        ...
        output = {
            ...
            'weights': weights,
            'depth_values_occ': depth_values_occ,
            'rgb_values_occ': rgb_values_occ,
            'weights_occ': weights_occ,
        }
        if self.rendering_network.color_encoder:
            output['rgb_direct_decoder_values'] = rgb_direct_decoder_values
        ...
        normals_occ = gradients_occ / (gradients_occ.norm(2, -1, keepdim=True) + 1e-6)
        normals_occ = normals_occ.reshape(-1, N_samples, 3)
        normal_map_occ = torch.sum(weights_occ.unsqueeze(-1) * normals_occ, 1)
        
        # transform to local coordinate system
        rot = pose[0, :3, :3].permute(1, 0).contiguous()
        normal_map_occ = rot @ normal_map_occ.permute(1, 0)
        normal_map_occ = normal_map_occ.permute(1, 0).contiguous()
        
        output['normal_map_occ'] = normal_map_occ
        # compute normal map
    ...
    def volume_rendering_occ(self, z_vals, occ):
        alpha = occ.reshape(-1, z_vals.shape[1])  # (batch_size * num_pixels) x N_samples

        weights = alpha * torch.cumprod(torch.cat([torch.ones((z_vals.shape[0], 1), 
                                                device=z_vals.device), 1.-alpha + 1e-10 ], -1), -1)[:, :-1]

...
class MonoSDFTrainRunner():
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                model_input["intrinsics"] = model_input["intrinsics"].cuda()
                model_input["uv"] = model_input["uv"].cuda() # [1, batch_size, 2]
                model_input['pose'] = model_input['pose'].cuda()
                ...
                loss_output = self.loss(model_outputs, model_input, ground_truth)
                ...
                if self.GPU_INDEX == 0:
                    ...
                    self.writer.add_scalar('Loss/rgb_direct_loss', loss_output['rgb_direct_loss'].item(), self.iter_step)
                    self.writer.add_scalar('Loss/virtual_normal_loss', loss_output['virtual_normal_loss'].item(), self.iter_step)
